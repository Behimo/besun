    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <nav class="flex h-16 items-center justify-between lg:h-20" aria-label="ناوبری اصلی">
            ...
            <div class="hidden items-center gap-1 lg:flex">
                @foreach (($navLinks ?? []) as $link)
                    @php
                        $sectionId = ltrim($link['href'], '#');
                    @endphp
                    <a
                        href="{{ $link['href'] }}"
                        class="nav-link"
                        :class="{ 'nav-link--active': $store.landing.activeSection === '{{ $sectionId }}' }"
                    >
            ...
            <div class="flex items-center gap-3">
                <a href="#contact" class="btn-demo hidden sm:inline-flex" data-magnetic>
        ...
        <div x-show="mobileMenu" x-cloak x-transition class="glass-nav mb-4 rounded-2xl p-4 lg:hidden">
            <div class="flex flex-col gap-1">
                @foreach (($navLinks ?? []) as $link)
                    <a href="{{ $link['href'] }}" @click="mobileMenu = false" class="rounded-xl px-4 py-3 text-sm text-slate-300 hover:bg-white/5 hover:text-white">
                        {{ $link['label'] }}
                    </a>
                @endforeach
                <a href="#contact" @click="mobileMenu = false" class="btn-demo mt-2 w-full justify-center">درخواست دمو</a>
            </div>
        </div>
    </div>
</header>
