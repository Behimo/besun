...
/**
 * Collect tabler icon names actually referenced in the project
 * instead of bundling the entire @iconify-json/tabler set (~2.7 MB CSS).
 */
function collectUsedTablerIcons(rootDir) {
    const icons = new Set();
    const scanRoots = [
        join(rootDir, 'resources/js'),
        join(rootDir, 'resources/styles'),
        join(rootDir, 'themeConfig.js'),
    ];
    const walk = (targetPath) => {
        if (!statSync(targetPath, { throwIfNoEntry: false })?.isDirectory()) {
            const content = readFileSync(targetPath, 'utf8');
            for (const match of content.matchAll(/tabler-([a-z0-9-]+)/g))
                icons.add(match[1]);
            return;
        }
        for (const entry of readdirSync(targetPath, { withFileTypes: true })) {
            ...
            if (entry.isDirectory())
                walk(entryPath);
            else if (/\.(vue|js|ts|jsx|tsx|scss|json)$/.test(entry.name))
                walk(entryPath);
        }
    };
...
const usedTablerIcons = collectUsedTablerIcons(projectRoot);
...
(async function () {
    ...
    console.log(`Saved CSS to ${relative(projectRoot, target)} (${usedTablerIcons.length} tabler icons)!`);
})().catch(err => {
    console.error(err);
});
/**
