 ...
 */
const canViewNavItem = item => (!item.action && !item.subject) || can(item.action, item.subject)

export const canViewNavMenuGroup = item => {
  const hasAnyVisibleChild = item.children.some(i => canViewNavItem(i))
...
export const canNavigate = to => {
  ...
  if (userData?.tenant?.isOwner || userData?.role === 'owner')
    return true
  ...
  // Routes without action/subject are open to any authenticated navigation check caller
