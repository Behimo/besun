...
const heroImage = '/marketing/crm-dashboard.png'
...
const translateMouse = computed(() => {
  ...
  const rotateX = (window.innerHeight - y.value) / 100
  const clampedRotateX = rotateX < -40 ? -20 : rotateX

  return {
    transform: `perspective(1200px) rotateX(${clampedRotateX}deg) rotateY(${(window.innerWidth - 2 * x.value) / 100}deg) scale3d(1,1,1)`,
  }
})
</script>
        ...
        <VContainer>
          <div class="hero-text-box text-center px-6">
            <h1 class="hero-title mb-4">
              {{ BRAND.nameFa }} — CRM فارسی برای تیم فروش شما
            </h1>
            <h6 class="mb-6 text-h6">
              لید، معامله، کمپین، قیف فروش و گزارش — در یک پنل SaaS چندمجموعه‌ای.
              {{ BRAND.trialDays }} روز آزمایش رایگان.
            </h6>
        ...
        <div class="hero-animation-img position-absolute">
          <RouterLink :to="{ name: 'login' }">
            ...
            >
              <img
                :src="heroImage"
                :alt="`پیشخوان ${BRAND.nameFa} — داشبورد CRM فارسی`"
                class="animation-img rounded-lg"
              >
            </div>
          </RouterLink>
        </div>
...
.hero-title {
  ...
  background: linear-gradient(135deg, #4A0E17 0%, #D4AF37 48%, #C75C6D 100%);
